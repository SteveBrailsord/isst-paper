...
cosmology, which we derive as the minimal inhomogeneous background
compatible with the field equations. A Branch theorem
...
as the critical next computation. For context: MOND (1983) did
not attempt CMB predictions for over twenty years;
TeVeS~\cite{Bekenstein1984,BekensteinTeVeS2004} (2004) required
several more years of independent analysis. ISST engages the CMB
...
construction is \PROVEN\ (foundations Layer~1, Stage~G gate clean).
...
not close as a derivation.} F145 tested four candidate paths and
ruled them all out:
\begin{enumerate}
\item $D_{\rm KL}$ between Maxwellians at $T_{\rm actual}$ and
$T_{\rm ref}$ (post-thermal-history vs.\ no-freezeout reference)
gives $1.36$ --- a factor $\sim 4$ short of $5.664$ at $\alpha = 0.869$.
\item $D_{\rm KL}$ between the post-recombination baryon Maxwellian
and the local-bounded uniform reference at $v_{\rm esc} = c$ gives
$\sim 40$--$60$ --- a factor $\sim 8$ too large; the local-bounded
reading is galactic-specific and lacks a natural cosmological
extension.
\item Per-step transient $D_{\rm KL}$ summed across the four
freezeouts gives $0.30$ --- a factor $\sim 19$ short; Shore--Johnson
additivity is over \emph{independent components}, not over
time-separated events.
\item R\'enyi-$q$ divergence summed across the four freezeouts
matches $5.66$ accidentally at $q = 2$ ($\sum D_2 = 5.82$), but the
Shore--Johnson uniqueness theorem forces $q = 1$ and the match
degrades at non-SM step sizes.
\end{enumerate}
...
would shift the prediction (working/predictions\_pending.md).
...
The committed paper position from F134 is that strict uniform-$f$
ISST is falsified at the third acoustic peak by Planck at $>10\sigma$
(Sec.~\ref{sec:theta}), and the CAMB Path~A treatment of
the $(1{+}\fprim)$ extra-matter contribution as effectively
collisionless CDM is the working approximation for the CMB prediction.
F138 derives the explicit functional form of the env-dependent $f$
closure that this approximation requires.
...
\textbf{Honest assessment of what this rescues, and what it does not.}
The closure
\eqref{eq:delta_f_linear}--\eqref{eq:env_dep_source} is a few-percent
multiplicative correction to the gravitational source. \emph{It does
not generate a free-growing collisionless component that decouples
from photon-baryon dynamics during radiation era}: $\delta f$ is a
linear functional of $\delta_b$ (and of $\delta\sigma$, which is
...
pre-decoupling exactly. The factor-33 third-peak suppression of strict
uniform-$f$ (F134, modified CLASS) is therefore \emph{not} structurally
rescued within F01 by this closure alone. A first-pass linear-scaling
estimate gives a residual gap of $\sim\!30$ relative to the
Planck-observed $H_3/H_1$ in either reading.

The CAMB Path~A treatment used in
Sec.~\ref{sec:theta}---modelling the $(1{+}\fprim)$
extra-matter contribution as effectively collisionless CDM at the
perturbation level---is therefore an \emph{empirical approximation}
rather than a derived consequence of F01. F138 confirms that the
env-dep-$f$ closure does not derive this approximation; it provides
a few-percent correction to it.

Two known resolution paths for the residual gap remain: (i) extend
the action with a $V(\Psi)$ potential generating an effective scalar
mass that decouples $\Psi$-mediated structure from photon dynamics
(an action extension beyond F01); (ii) the position the paper now
adopts in Sec.~\ref{sec:theta}, that the CMB peak
structure is not fully derivable from F01 alone, with the Path~A
agreement reported as empirical and ISST's distinguishing predictions
concentrated in late-time observables. Status: \PROVEN\ for the
closure form
\eqref{eq:delta_f_linear}--\eqref{eq:env_dep_source} and the $\beta(z)$
values \eqref{eq:beta_values}; \DEMO\ for the leading-order suppression
of the $\Lambda$-correction $\Delta^f_{\mu\nu}$ pending explicit
evaluation of $\bar\Lambda$ (F138 §8); \OPEN\ (F138 working on) for
a derivation of the CAMB Path~A proxy from F01 alone---the residual
question that would close the F134 falsification within the committed
action.
...
substituting into the Ψ-transport amplitude balance
...
\textbf{vs.\ $\Lambda$CDM.} $\Lambda$CDM fits all six of
$(\Omega_b, \Omega_c, H_0, \tau, A_s, n_s)$ from CMB; ISST has
$a_{\rm crit}$ derived from the F01 action and $\fprim$ as an
SM-motivated ansatz, plus seven empirical inputs and one inheritance
from Wiltshire. ISST's empirical regulator parameters
$(a_0^{\rm reg}, q)$ are fitted on rotation curves only and do not
propagate into the cosmological prediction. $\Lambda$CDM
\emph{assumes} the dark sector and asks ``what density?''; ISST
proposes $(1+\fprim)\Omega_b = \Omega_m$ as the cosmological-floor
identity and asks ``what observation could break it?''. The total
parameter count favours $\Lambda$CDM (6 vs 10); the
derived-vs-fitted axis favours ISST (2 from first principles vs 0).
...
recover $5.66$) is identified as a propagated sub-problem
(working/predictions\_pending.md). \OPEN.
...
no kinetic term for the scalar. From that action it derives
the Branch theorem, which routes Solar-System physics into Branch~B
(exact GR, via the static-fluid theorem of
App.~\ref{app:static_fluid_theorem}) and cosmology into Branch~A
(where homogeneous FRW fails geometrically and a Wiltshire-class
inhomogeneous background is the simplest physically motivated
realisation; strict exclusion of LTB/Szekeres alternatives is
identified as open); the cluster mass identity
$\Omega_m = (1{+}\fprim)\Omega_b$ at $1.6$--$1.8\sigma$ tension
with the SM-predicted $\fprim = 5.66\pm 0.12$; PPN
$\gamma=\beta=1$ exactly; $|\dot G/G|$ five orders below LLR;
$c_{\rm GW}=c$ structurally rather than by tuning, from the F01
linear-in-$\Psi$ gravitational sector with no kinetic term --- two
tensor degrees of freedom and zero propagating scalar mode
(App.~\ref{app:adm_dof}, internal, external verification flagged);
SBBN inherited; a competitive fit to 175 SPARC rotation curves
(held-out cross-validation on the 161 galaxies outside the
14-galaxy tuning set preserves the central $Q{=}1$ comparison;
F147); a derived $a_{\rm crit}$ sitting 11\% from MOND $a_0$ at
the committed $H_0$; $f\sigma_8$ at $p\approx 0.30$--$0.45$ on the current RSD
survey set; an $H_0$ tension whose \emph{structure} is reproduced
as a Planck-template-vs-SH0ES-ladder pipeline artefact (ISST gives
$62.8$ vs.\ Planck $67.4$, and $70.6$ vs.\ SH0ES $73.0$ ---
structure recovered, literal numerical recovery open); and a
lighthouse Bullet mechanism that closes the bulk of the
local-column gap with two empirical $f_s$ values and predicts
both Bullet and Abell~520 with the same machinery.

ISST \emph{eliminates dark matter as a particle species}: there
are no WIMPs, axions, or sterile neutrinos in the theory; the
gravitational effects attributed to dark matter arise from the
information-enhanced baryon coupling $(1{+}f)$. The CMB
perturbation structure under the strict F01 action remains open:
the first-pass acoustic angle within $0.83\%$ of Planck on a
$(w_0, w_a)$ proxy for the Wiltshire-effective background uses
an empirical proxy that represents the enhanced coupling as a
collisionless component in standard Boltzmann codes (CAMB Path~A,
Sec.~\ref{sec:theta}). This is a computational limitation of the
tool, not a theoretical retreat; the proxy is needed because
standard Boltzmann codes know how to handle extra gravitational
source only through CDM-like components. The proxy diverges from
the committed Wiltshire $H(z)$ by $+76\%$ at recombination, so the
sub-percent headline carries an estimated $\sim 5$--$10\%$
systematic uncertainty; a Boltzmann code with the two-domain
background is the next critical computation. A strict uniform-$f$
perturbation source is falsified at the third CMB peak by
$>10\sigma$; the linearised env-dep-$f$ closure provides a
few-percent correction but does not structurally rescue the
third-peak gap. ISST's distinguishing predictions live in
late-time structure formation: the lighthouse mechanism in
cluster mergers, the $f\sigma_8$ shape on the Wiltshire
background (DESI~DR2 comparison identified as the immediate
test), the void-lensing $G$-ratio, and the
processing-history-dependent $f_s$ in jellyfish galaxies and UDGs.

ISST has not produced: a derivation of the CMB peak-height ratios
from the F01 action; a Boltzmann computation on the true Wiltshire
two-domain background; a derivation of the regulator exponent $q$;
a closed chain from the action's KL functional to the discrete-sum
$\fprim$ (four candidate paths ruled out, Sec.~\ref{sec:fprim});
a single-$\alpha$ scale-bridging calibration closing the
factor-$\sim 2$ galactic-vs-cosmological gap
(Sec.~\ref{sec:alpha_scale}); a derivation of the lighthouse
$f_s^\star$ from violent-relaxation working memory; or a derived
matter power spectrum $P(k)$.

ISST has dropped, on internal audit: the $S_8$ prediction (wrong
sign); the halo-ontology Bullet attempts (internal references
F65/F66/F68); the F113 BBN catastrophe (wrong-background
artefact); the $-12\%$ analytical $\theta_*$ estimate (superseded
by the first-pass Boltzmann); strict uniform-$f$ perturbation
evolution (third-peak falsification, $>10\sigma$); and the
smooth-crossover $\fprim$ reduction (smoothing returns $5.78$, not
$5.40$).

The kill conditions are pre-committed: direct dark-matter particle
detection (WIMP, axion, sterile-neutrino specifications in
Sec.~\ref{sec:falsification}), fifth-force detection
(composition-dependent acceleration anomaly or Yukawa-type force on
test particles), $c_{\rm GW}\neq c$, $\gamma_{\rm PPN}$ deviation
at $10^{-5}$, BBN inconsistent with SBBN, Euclid finding $\kappa$
tracking gas in cluster mergers, DESI~DR2 finding $f\sigma_8(z)$
within $1\sigma$ of $\Lambda$CDM, and Euclid finding
$G_{\rm void}/G_{\rm local}=1$ to $0.5\%$. Each is a single
measurement, falsifiable in this decade.

\textbf{Self-containment and reproducibility.} This paper is
self-contained: every load-bearing derivation is presented in the
body or appendices (App.~\ref{app:passport}--\ref{app:adm_dof}).
The companion site at
\href{https://lily-labs.co.uk/isst}{lily-labs.co.uk/isst} provides
supplementary computational outputs, the interactive Derivation
Passport engine, and extended audit logs. Boltzmann code
modifications (the CAMB \texttt{params.ini} configurations
underlying Sec.~\ref{sec:theta} and the modified CLASS
\texttt{source/perturbations.c} patch implementing the strict
uniform-$f$ test) and the SPARC analysis scripts will be archived
as supplementary material with the publisher and on a permanent
repository (Zenodo) upon acceptance. The invitation is open:
here are our kill conditions; try to trigger them.
...
step of the anisotropic-to-isotropic transition. It addresses the
referee's request for ``a clearer bridge between the anisotropic
wall slab and the averaged Friedmann equation to ensure no
energy-momentum is lost in the averaging process.''
The full derivation, including symbolic verification with
\textsc{sympy}, is in
\texttt{working/F141\_wall\_friedmann\_proof.md} and
\texttt{working/F141\_em\_conservation\_check.py}.
...
\eqref{eq:eom}.
\end{itemize}
This closes Item~2 of the second-round referee report.
...
The retroactive audit (2026-04-26) classifies all 88 audited results:
...
\bibitem{Padmanabhan2010} T.~Padmanabhan, ``Thermodynamical aspects of
